...
function checkActiveForm(oForm){
	...
	//if (eval(window.document.all.activeforminfo)!=null){
		...
		for (i=0; i < allInputs.length; i++) {
			if(allInputs[i].getAttribute('activeform')!=null){
				if (allInputs[i].getAttribute('activeform') == "true") {
					var fieldValue = "";
					...
					if (allInputs[i].type.toUpperCase() == "HIDDEN" || allInputs[i].type.toUpperCase() == "TEXT" || allInputs[i].type.toUpperCase() == "NUMBER" ) {
						...
						/*result = activeFormCreateInfoLine(
							...
							allInputs[i].getAttribute('required'),
							fieldValue,
							allInputs[i].getAttribute('minsize'),
							allInputs[i].getAttribute('maxsize'),
							allInputs[i].getAttribute('inputtype'),
							collectionName
						);*/
						result = activeFormCreateInfoLine(
							allInputs[i].getAttribute('name'),
		...
		for (i=0; i < allSelects.length; i++) {
			if(allSelects[i].getAttribute('activeform')!=null){
				if (allSelects[i].getAttribute('activeform') == "true") {
					var fieldValue = "";
					...
					result = activeFormCreateInfoLine(
						allSelects[i].getAttribute('name'),
						allSelects[i].getAttribute('required'),
						fieldValue,
						allSelects[i].getAttribute('minsize'),
						allSelects[i].getAttribute('maxsize'),
						allSelects[i].getAttribute('inputtype'),
						collectionName
					);
		...
		for (i=0; i < alltextarea.length; i++) {
			if(alltextarea[i].getAttribute('activeform')!=null){
				if (alltextarea[i].getAttribute('activeform') == "true") {
					var fieldValue = "";
					...
					if (alltextarea[i].type.toUpperCase() == "TEXTAREA") {
						...
						result = activeFormCreateInfoLine(
							alltextarea[i].getAttribute('name'),
							alltextarea[i].getAttribute('required'),
							fieldValue,
							alltextarea[i].getAttribute('minsize'),
							alltextarea[i].getAttribute('maxsize'),
							alltextarea[i].getAttribute('inputtype'),
							collectionName
						);
	...
	if(required == "true") {
		...
		if (oRadio.checked == "true"){//CHECKS THE SELECTED RADIO
			...
			for (l=0; l < collection.length; l++) {
				if(collection[l].getAttribute('activeform')!=null){
				if (collection[l].getAttribute('activeform') == "true") {
