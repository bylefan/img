...
var ie9		= !opera && /msie (9|1[0-9])/i.test(ua);
var ieBox = ie && (document.compatMode == null || document.compatMode != "CSS1Compat");
var moz		= !opera && /mozilla\/[56789]/i.test(ua) && !ie;
var kon   = /Konqueror/i.test(ua);
var lin   = /Linux/i.test(ua);
var mod   = ie9 || moz || chrome || opera;
...
//		alertDEBUG("show_sub("+menuName+","+id+","+y+")");
...
//				if(moz) offsetLeft += menuInfo.btn.paddingLeft-0+menuInfo.btn.paddingRight-0;
			}else{
...
//				if(moz) offsetLeft -= menuInfo.btn.paddingLeft-0+menuInfo.btn.paddingRight-0;
	...
	{
//		alertDEBUG("show_submenu("+menuName+","+id+","+x+","+y+")");
...
//		if(moz) {
//			btnWidth += menuInfo.btn.paddingLeft-0+menuInfo.btn.paddingRight-0;
//			btnHeight += menuInfo.btn.paddingTop-0;
//		}
...
var layout = {
	...
	getHeight : function(el) {
		return (mod) ? el.offsetHeight:el.offsetHeight;
	},
	...
	for(i=0;i<this.items.length;i++)
	{
		...
		else
		{
			...
			this.s += "<a menuname='"+menuInfo.menuName+"' id='"+this.items[i].id+"' "+
						...
						(this.items[i].isSub ? "MenuHandler.show_sub(this."+(mod?"getAttributeNode(\"menuname\").nodeValue":"menuname")+",\""+this.items[i].action.id+"\",this);":"")+
...
MenuBar.prototype.toString = function() {
	...
	for(i=0;i<this.obj.length;i++)
	{
		...
		if (this.obj[i].isMenu) 
			_pos = "MenuHandler.show_submenu(this."+(mod?"getAttributeNode('menuname').nodeValue":"menuname")+",'"+this.obj[i].id+"',"+"layout.getX(this)-"+_k+",layout.getY(this)-"+_k+");"+_changeBtnLook;
		else 
			_pos = _changeBtnLook;
		...
		_da = (!this.obj[i].image && this.obj[i].isMenu) ? "<img border=0 class='"+menuInfo.menuName+"down_arrow' src='"+_arrowD.object.src+"' alt=''>":"";
		...
		// ADDING ONE BUTTON
		////////////////////////////////
		ret += "<Table style='cursor:hand;display:"+buttonDisplay+"' dir='"+menuInfo.menudir+"' cellpadding=0 cellspacing=0 border=0><Tr>";
...
function getDhtmlMenuStyle(menuInfo){
	...
	if(menuInfo.version == undefined || mod){
	...
	if(mod) {
		menuInfo.btn.width -= (menuInfo.btn.paddingLeft-0)+(menuInfo.btn.paddingRight-0);
		menuInfo.btn.height -= menuInfo.btn.paddingTop-0;
	
		menuInfo.hotbarWidth -= (menuInfo.hotbarPaddingLeft-0)+(menuInfo.hotbarPaddingRight-0);
		menuInfo.hotbarHeight -= menuInfo.hotbarPaddingTop-0;
	}
	
//	if(!menuInfo.btn.paddingTopHot)
//		menuInfo.btn.paddingTopHot = menuInfo.btn.paddingTop;
//	if(!menuInfo.btn.paddingLeftHot)
//		menuInfo.btn.paddingLeftHot = menuInfo.btn.paddingLeft;
//	if(!menuInfo.btn.paddingRightHot)
//		menuInfo.btn.paddingRightHot = menuInfo.btn.paddingRight;
//	if(!menuInfo.btn.paddingBottomHot)
//		menuInfo.btn.paddingBottomHot = menuInfo.btn.paddingBottom;
	...
	if(mod) {
		if(menuInfo.btn.textSize == 'small')
			menuInfo.btn.textSize = 'medium';
		else if(menuInfo.btn.textSize == 'x-small')
			menuInfo.btn.textSize = 'small';
		else if(menuInfo.btn.textSize == 'xx-small')
	...
	if(opera || chrome) {
		sCode += "."+menuInfo.menuName+"mMenuBar > table {\n";
		if (menuInfo.menudir == 'rtl')
			sCode += "	float:right;\n";
		else
			sCode += "	float:left;\n";
	...
	if(mod)
		sCode += "	min-width:"+menuInfo.btn.width+"px;\n";
	else
		sCode += "	width:"+menuInfo.btn.width+"px;\n";
	if(mod)
		sCode += "	min-height:"+menuInfo.btn.height+"px;\n";
	else
		sCode += "	height:"+menuInfo.btn.height+"px;\n";
	if(mod)
		sCode += "	line-height:"+(menuInfo.btn.height-menuInfo.btn.paddingTop-menuInfo.btn.paddingBottom)+"px;\n";
	...
	sCode +="	."+menuInfo.menuName+"TButton_hover {\n";
		...
		if(mod)
			hc += "	min-width:"+menuInfo.btn.width+"px;\n";
		else
			hc += "	width:"+menuInfo.btn.width+"px;\n";
		if(mod)
			hc += "	min-height:"+menuInfo.btn.height+"px;\n";
		else
			hc += "	height:"+menuInfo.btn.height+"px;\n";
		if(mod)
			hc += "	line-height:"+(menuInfo.btn.height-menuInfo.btn.paddingTopHot-menuInfo.btn.paddingBottomHot)+"px;\n";
	...
	sCode += "	."+menuInfo.menuName+"TButton_selected {\n";
		...
		if(menuInfo.btn.backSelectedColor == undefined &&
			...
			if(!mod)
				...
				sc += "	min-width:"+menuInfo.btn.width+"px;\n";
			if(!mod)
				sc += "	height:"+menuInfo.btn.height+"px;\n";
			else
				sc += "	min-height:"+menuInfo.btn.height+"px;\n";
			if(mod)
				sc += "	line-height:"+(menuInfo.btn.height-menuInfo.btn.paddingTopSelected-menuInfo.btn.paddingBottomSelected)+"px;\n";
...
//	alert(sCode);
	...
	function private_getMenuCodeByName(menuObjName){
		...
		js += "	"+menuObjName+".designType = '"+this.designType+"';";
